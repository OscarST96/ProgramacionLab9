...
public class Enemy : MonoBehaviour
{
    ...
    private void OnFollowSeparate()
    {
        ...
        Vector3 separation = Vector3.zero;//Este vector acumular� la fuerza de separaci�n respecto a los otros enemigos.

        Enemy[] allEnemies = FindObjectsOfType<Enemy>();//Busca todos los enemigos en la escena, para calcular separaci�n con cada uno.
        
        //Itera sobre todos los enemigos.
        foreach (Enemy other in allEnemies)
        {
            if (other != this)//Ignora a s� mismo
            {
                ...
                if (distance < separationRadius)//Si la distancia es menor que el radio establecido
                {
                    ...
                    separation += (transform.position - other.transform.position).normalized / distance;//Cuanto m�s cerca est�n, mayor ser� la fuerza de repulsi�n
                }
            }
        }
        //Combina la direcci�n hacia el jugador con la fuerza de separaci�n de los dem�s enemigos.
        Vector3 finalDirection = (directionToPlayer + separation).normalized;
        rb.linearVelocity = finalDirection * speed;//Mueve al enemigo con la velocidad deseada en la direcci�n calculada.
    }
    #endregion
}
